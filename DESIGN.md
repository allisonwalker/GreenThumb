...
**Strongest existing move:** the Persuade forest field — full-viewport forest (`#172217`) with cream (`#f7faf7`) type, oversized bold headlines, shell off. Named tokens for that motif live in `app/globals.css`. Signed-in **chrome** now uses forest/cream at Operate density (usable nav, not a fold-covering poster). **Today, Garden, and Catalog page titles** now use that type conviction at list/form density. Log and Ask bodies stay quiet until their bolder tickets.
...
That is one hierarchy step above body copy on destinations that have not had a bolder ticket yet. It is quieter than landing’s `clamp` headlines. **Today, Garden, and Catalog** are the exception: page titles use `text-5xl` / `sm:text-6xl`, `leading-none`, and `tracking-display` in `text-forest` — type conviction at Operate density, not `text-display`.

**Color and density:** cream `--background` shows at the edges of main; content sits on **white cards** (`rounded-2xl` or shadcn `rounded-xl`, `border`; Log / Ask still often `shadow-sm`). Chrome is forest/cream. Log and Ask still use mixed greens (`green-700` eyebrows, `green-800` primary buttons / links, `green-50` washes) plus Tailwind `neutral-*`. Today, Garden, and Catalog page bodies use motif names. All of them read as a **dashboard / form / list**, not a marketing fold.
...
**Today, Garden, and Catalog** now share landing’s motif and type conviction at Operate density. Log and Ask page bodies **still read quieter than landing** until their own tickets. Same product: Persuade fills the fold; chrome is forest/cream; Today, Garden, and Catalog titles are the Operate peak; remaining Operate pages are still modest type + default cards.

## Intent for later bolder (page tickets)

Operate chrome already shares landing’s **motif and type conviction** at **Operate density**. **Today, Garden, and Catalog titles** do too (`tracking-display`, not `text-display`). Log and Ask still read quieter than landing until their own tickets. Do not restyle `/` or `/sign-in` again as a stand-in. Do not invent new hex or a new font in a page ticket — use `forest` / `cream` / `leaf` / `leaf-muted` / `selection` / `text-display` / `tracking-display`. Recapture this file after remaining page bolders ship.
...
**Tailwind on Operate page bodies:** Log and Ask still use `neutral-*`, `green-50` / `green-200` / `green-700` / `green-800` / `green-900`, `red-*`, `white`, default `border`. **Today** uses motif names (`forest`, `cream`, `selection`) plus `amber-*` for matching error. **Garden** uses motif names (`forest`, `cream`, `leaf`, `selection`) plus `amber-*` for out-of-season, `red-*` for remove/errors, and sun-map legend colors (`amber-300` / `lime-300` / `emerald-300` / `teal-500`). **Catalog** uses motif names (`forest`, `cream`, `leaf`, `selection`) plus `red-*` for save/create errors. Chrome uses `forest` / `cream` / `leaf` / `selection`.
