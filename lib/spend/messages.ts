...
export function monthlyCapAlertMessage(thresholdUsd: number, spendUsd: number) {
  return `Jory Journal paid LLM spend has crossed $${thresholdUsd.toFixed(0)} (about $${spendUsd.toFixed(2)} so far this month). Conversation and crop drafts are capped at $100; matching / the morning care list does not use the model.`;
}
